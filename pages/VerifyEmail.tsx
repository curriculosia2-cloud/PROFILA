...
const VerifyEmail: React.FC<VerifyEmailProps> = ({ userEmail }) => {
  ...
  const handleResend = async () => {
    ...
    setResending(true);
    try {
      await supabaseService.resendConfirmationEmail(userEmail);
      setSent(true);
      setTimeout(() => setSent(false), 5000);
    } catch (err) {
      alert("Erro ao reenviar e-mail. Tente novamente em alguns instantes.");
    } finally {
      setResending(false);
    }
  };
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-6 bg-slate-50">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-[3rem] shadow-2xl p-12 border border-slate-100 text-center">
            <div className="bg-blue-50 w-20 h-20 rounded-[2rem] flex items-center justify-center mx-auto mb-8 shadow-inner text-brand-blue">
                <Mail size={40} />
            </div>
            
            <h2 className="text-4xl font-black text-brand-dark tracking-tighter mb-4">Confirme seu <span className="text-brand-blue">E-mail</span></h2>
            <p className="text-slate-500 font-medium leading-relaxed mb-10">
                Enviamos um link de confirmação para {userEmail ? <strong className="text-brand-dark">{userEmail}</strong> : 'seu e-mail'}.<br/>
                Por favor, clique no botão no e-mail para ativar sua conta PROFILA.
            </p>
