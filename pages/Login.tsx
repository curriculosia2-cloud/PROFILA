    ...
    <div className="min-h-[calc(100vh-80px)] flex items-center justify-center p-6 bg-slate-50">
      <div className="max-w-md w-full">
        ...
        <div className="bg-white rounded-[2.5rem] shadow-2xl p-10 border border-slate-100">
            ...
            {error && (
                <div className={`mb-6 p-5 rounded-2xl border animate-in fade-in zoom-in duration-300 ${error.type === 'unconfirmed' ? 'bg-amber-50 border-amber-100' : 'bg-red-50 border-red-100'}`}>
                    <div className="flex items-center space-x-3 mb-2">
                        <AlertCircle className={`h-5 w-5 ${error.type === 'unconfirmed' ? 'text-amber-500' : 'text-red-500'}`} />
                        <span className={`text-xs font-black uppercase tracking-widest ${error.type === 'unconfirmed' ? 'text-amber-600' : 'text-red-500'}`}>Atenção</span>
                    </div>
                    <p className={`text-sm font-medium ${error.type === 'unconfirmed' ? 'text-amber-700' : 'text-red-600'}`}>
                        {error.message}
                    </p>
                    {error.type === 'unconfirmed' && (
                        <button 
                            ...
                            disabled={resending}
                            className="mt-4 text-[10px] font-black uppercase tracking-[0.2em] text-amber-800 hover:underline flex items-center disabled:opacity-50"
                        >
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-3">
                  ...
                  <input 
                      ...
                      className="w-full pl-12 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition-all font-medium text-sm"
              ...
              <div className="space-y-3">
                  ...
                  <input 
                      ...
                      className="w-full pl-12 pr-12 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-brand-blue focus:border-transparent outline-none transition-all font-medium text-sm"
                      placeholder="••••••••"
                  />
                  <button 
                      type="button"
            ...
            <div className="my-8 flex items-center space-x-4">
              ...
              <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest">Ou entre com</span>
              <div className="flex-1 h-px bg-slate-100"></div>
            </div>
